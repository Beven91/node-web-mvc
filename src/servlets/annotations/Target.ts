...
Target.install = function <A, T>(ctor: A): CallableAnnotationFunction<A, T> {
    ...
    if (elementType === 'UNKNOW') {
      ...
      return (...params) => {
        ...
        if (params[0] === parameterReturnable) {
          ...
          // 创建注解
          return createAnnotation(runtime, ctor);
        }
        // 创建注解 此处不可返回
        createAnnotation(runtime, ctor);
      }
    }
    // 没有进行配置
    runtime.options = [];
  ...
  decorator.prototype = ctor;
  decorator.Class = ctor;
  Object.defineProperty(decorator, 'name', { value: (ctor as any).name });
  return decorator as any;
}

function createAnnotation(runtime: AnnotationOptions, ctor): RuntimeAnnotation {
  const target = runtime.meta[0];
  runtime.types = target[elementTypes] || target.constructor[elementTypes] || [];
  // 创建注解
  return RuntimeAnnotation.create(runtime, ctor)
}