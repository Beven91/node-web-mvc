...
export default class Schemas {
  ...
  private buildApiModelProperties(clazzType: ClazzType) {
    ...
    for (const key of keys) {
      ...
      const metaRuntimeType = RuntimeAnnotation.getPropertyAnnotation(clazzType, key, MetaRuntimeType)?.nativeAnnotation;
      ...
      const dataType = metaRuntimeType?.value || anno?.dataType || basicProperty?.dataType || anno.example?.constructor;
